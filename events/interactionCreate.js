...
return interaction?.reply({ content: "Taxa limitada", ephemeral: true })
} else {
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 Entra no canal de voz primeiro`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `🔴 Tem que estar no mesmo canal de voz`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `▶️ Você não tem os seguintes molhos: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`, ephemeral: true });
            }
            } catch (e) {
