...
module.exports = {
  name: "filter",
  description: "Adds audio filter to ongoing music. Adiciona filtros de áudio para as músicas que tão rolando",
  ...
  run: async (client, interaction) => {
    try {
      ...
      .setDescription('** Escolha uma das opções e veja a mágica acontecer**')
...
if(!filters?.includes(button?.customId)) return
     ...
     filtre = filtre?.toLowerCase()
      ...
      if (filters?.includes(filtre?.toLowerCase())) {
        ...
        embed?.setDescription(`❌ Não achei esse filtro`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))
