  ...
  run: async (client, interaction) => {
    ...
    try {
     ...
     const queue = client.player.getQueue(interaction.guild.id);
      ...
      if (!queue.songs[0]) return interaction.reply({ content: '⚠️ A fila tá vazia', ephemeral: true }).catch(e => { })
      ...
      const generateEmbed = async (start) => {
        ...
        if (!current || !current?.length > 0) return interaction.reply({ content: '⚠️ A fila tá vazia', ephemeral: true }).catch(e => { })
        return new EmbedBuilder()
          .setTitle(`${interaction.guild.name}  Queue`)
          .setThumbnail(interaction.guild.iconURL({ size: 2048, dynamic: true }))
          .setColor(client.config.embedColor)
          .setDescription(`▶️ Pedrada atual: \`${queue.songs[0].name}\`
    ${current.map(data =>
      ...
      await interaction.reply({
        ...
        collector.on("end", async (button) => {
          ...
          const embed = new EmbedBuilder()
            .setTitle('Tempo do comando esgotado')
            .setColor(`#ecfc03`)
            .setDescription('▶️ Execute o comando de fila denovo')
