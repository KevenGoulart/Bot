...
module.exports = {
  name: "play",
  description: "Bora ouvir umas pedrada",
  permissions: "0x0000000000000800",
  options: [{
    name: 'name',
    description: 'Escreva a música que quer',
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!name) return interaction.reply({ content: `❌ Insira um nome válido`, ephemeral: true }).catch(e => { });
      let res;
      try {
      ...
      interaction.reply(code).then(async Message => {
        ...
        collector.on('collect', async (button) => {
          switch (button.customId) {
            case 'cancel': {
              embed.setDescription(`Procura interrompida`);
