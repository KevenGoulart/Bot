...
module.exports = {
  ...
  options: [{
    name: "number",
    description: "Insira quantas músicas quer pular",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (number) {
        if (!queue.songs.length > number) return interaction.reply({ content: '⚠️ Número de músicas excedido', ephemeral: true }).catch(e => { })
        ...
        return interaction.reply({ content: '❌ A fila tá vazia', ephemeral: true }).catch(e => { })
      }
      } else {
try {
  const queue = client.player.getQueue(interaction.guild.id);
  ...
  const embed = new EmbedBuilder()
    .setColor('#3498db')
    .setAuthor({
      name: 'Música pulada (não era tão pedrada assim)',
    ...
    .setDescription(success ? ` **Pulada** : **${old.name}**` : '❌ A fila tá vazia')
    .setTimestamp();
